    ...
    for epoch in range(opt.num_epochs):
        ...
        if opt.region_relation == 'NTN_relation':
            save_checkpoint({
                ...
                'opt': opt,
                'Eiters': model.Eiters,
            }, is_best, filename='checkpoint_{}.pth.tar'.format(epoch), prefix=opt.model_name + '/' + opt.region_relation + '/')
            ...
            save_checkpoint({
                ...
                prefix=opt.model_name +'/' + opt.region_relation + '/')
...
def validate(opt, val_loader, model):
    ...
    img_embs, cap_embs, bboxes, cap_lens = encode_data(
        model, val_loader, opt.log_step, logging.info)

    img_embs = numpy.array([img_embs[i] for i in range(0, len(img_embs), 5)])

    start = time.time()
    sims = shard_xattn(model, img_embs, cap_embs, bboxes, cap_lens,
