...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, bboxes, lengths, ids) in enumerate(data_loader):
    ...
    for i, (images, captions, bboxes, lengths, ids) in enumerate(data_loader):
        ...
        del images, captions
    return img_embs, cap_embs, all_bbx,  cap_lens
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    img_embs, cap_embs, bbox, cap_lens = encode_data(
        model, data_loader)
    print('Images: %d, Captions: %d' %
          (img_embs.shape[0] / 5, cap_embs.shape[0]))

    path = opt.model_name + '/' + opt.region_relation + '/'
    ...
    if not fold5:
        ...
        sims = shard_xattn(model, img_embs, cap_embs, bbox,
                           cap_lens, opt, shard_size=80)

        # save similarity matrix for the fusion of different model
        # np.save(path + '%s_%s_sim.npy' % (opt.region_relation, opt.windows_size), sims)
        ...
        for i in range(5):
            ...
            sims = shard_xattn(model, img_embs_shard, cap_embs_shard,
                               bbox_shard,  cap_lens_shard, opt, shard_size=80)

            #np.save(identify + 'sim_matrxi.npy', sims)
            end = time.time()
            print("calculate similarity time:", end - start)
...
def shard_xattn(model, images, captions, bbox, caplens, opt, shard_size=128):
    """
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            sim = model.forward_sim(im, s, bbx, l)
